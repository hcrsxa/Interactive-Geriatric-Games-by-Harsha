...
namespace OpenCVForUnityExample
{
    ...
    public class ObjectDetectionNanoDetPlusExample : MonoBehaviour
    {
...
#if OPENCV_SENTIS_AVAILABLE
        [Tooltip("When enabled, runs NanoDet+ inference with Sentis (MultiBackendDnn.DNN_BACKEND_UNITY_SENTIS). Inspector paths may stay .onnx; at runtime they are rewritten to .sentis and loaded from StreamingAssets (place a matching .sentis beside the onnx file).")]
        public bool UseSentisInference = true;
        [Tooltip("When using Sentis: backend / target selects Sentis BackendType (CPU / GPU, etc.).")]
        public BackendType NanoDetSentisBackendType = BackendType.GPUCompute;
#endif
        ...
        // Private Fields
        private NanoDetPlusObjectDetector _objectDetector;
        ...
        private void Run()
        {
            ...
            // NanoDet+ object detector (uses dnn/*.onnx / .sentis model under StreamingAssets).
            InitializeInference();
        ...
        public async void OnSentisBackendDropdownValueChanged(int index)
        {
            ...
            if (newBackend == NanoDetSentisBackendType)
                return;
            ...
            NanoDetSentisBackendType = newBackend;
            UpdateUseSentisInference();
            UpdateUseAsyncInference();
...
#if OPENCV_SENTIS_AVAILABLE
        /// <summary>
        /// Aligns the dropdown with <see cref="NanoDetSentisBackendType"/> without raising change events. Option order must match <see cref="SentisBackendTypesInEnumOrder"/>.
        /// </summary>
        private void UpdateSentisBackendDropdown()
        {
            ...
            if (SentisBackendTypesInEnumOrder.Length == 0)
                return;
            int idx = Array.IndexOf(SentisBackendTypesInEnumOrder, NanoDetSentisBackendType);
            if (idx < 0)
                idx = 0;
        ...
        /// <see cref="NanoDetSentisBackendType"/> is <see cref="BackendType.GPUCompute"/>, sets <see cref="NanoDetSentisBackendType"/> to <see cref="BackendType.GPUPixel"/>.
        /// </summary>
        private void UpdateUseSentisInference()
        {
#if OPENCV_SENTIS_AVAILABLE
            if (!SystemInfo.supportsComputeShaders && NanoDetSentisBackendType == BackendType.GPUCompute)
                NanoDetSentisBackendType = BackendType.GPUPixel;
#endif
        }
        ...
        /// <summary>
        /// Disposes the inference runner and <see cref="NanoDetPlusObjectDetector"/> asynchronously (used from <see cref="OnDestroy"/> and when switching backends).
        /// </summary>
        ...
        private void InitializeInference()
        {
            ...
            try
            {
                ...
                if (UseSentisInference)
                {
                    _objectDetector = new NanoDetPlusObjectDetector(
                        modelPath,
                        _classesFilepath,
                        ...
                        (int)NanoDetSentisBackendType);
                    Debug.Log("NanoDetPlusObjectDetector initialized (Sentis / DNN_BACKEND_UNITY_SENTIS, backend=" + NanoDetSentisBackendType + ").");
                }
                else
#endif
                {
                    _objectDetector = new NanoDetPlusObjectDetector(modelPath, _classesFilepath, new Size(InpWidth, InpHeight), ConfThreshold, NmsThreshold, TopK);
                    Debug.Log("NanoDetPlusObjectDetector initialized (OpenCV DNN).");
                }
            ...
            {
                Debug.LogWarning("ObjectDetectionNanoDetPlusExample InitializeInference failed: " + ex);
            }
        }
