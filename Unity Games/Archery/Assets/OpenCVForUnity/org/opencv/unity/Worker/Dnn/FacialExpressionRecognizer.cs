...
namespace OpenCVForUnity.UnityIntegration.Worker.DnnModule
{
    ...
    /// </summary>
    public class FacialExpressionRecognizer : DnnInferenceWorkerBase
    {
            ...
            : base(backend, target)
        {
            ...
            _facialExpressionRecognitionNet.setPreferableBackend(DnnBackend);
            _facialExpressionRecognitionNet.setPreferableTarget(DnnTarget);
        ...
        ///
        /// For better performance in async scenarios, use RecognizeTaskAsync instead.
        /// </remarks>
        ...
        /// Web builds cannot use thread pools; only then does inference run synchronously on the caller thread.
        /// </remarks>
        /// <param name="image">Input image in BGR format.</param>
        /// <param name="faceBox">Optional face detection result from YuNetV2FaceDetector. If null, the entire image area will be used for inference.</param>
        ...
        public virtual async Task<Mat> RecognizeTaskAsync(Mat image, Mat faceBox = null, CancellationToken cancellationToken = default)
        {
            await ExecuteTaskAsync(image, faceBox, cancellationToken);
            return CopyOutput();
        }
        ...
        /// </summary>
        /// <remarks>
        /// <c>@deprecated</c> Use <see cref="RecognizeTaskAsync(Mat, Mat, CancellationToken)"/>. In a future version, this member will return Unity <c>Awaitable</c> instead of <see cref="Task{TResult}"/>.
        /// See <see cref="RecognizeTaskAsync(Mat, Mat, CancellationToken)"/>. Web synchronous fallback applies when thread pools are unavailable (OpenCV Dnn inference only).
        /// </remarks>
        [Obsolete("Use RecognizeTaskAsync(). RecognizeAsync() will return Awaitable in a future version.")]
        public virtual Task<Mat> RecognizeAsync(Mat image, Mat faceBox = null, CancellationToken cancellationToken = default) =>
            RecognizeTaskAsync(image, faceBox, cancellationToken);
        ...
        protected override Mat[] RunCoreProcessing(Mat[] inputs)
        {
            ...
            // Preprocess & forward
            Mat outputBlob;
            using (Mat inputBlob = PreProcess(image, faceBox))
            {
        ...
        protected override Task<Mat[]> RunCoreProcessingTaskAsync(Mat[] inputs, CancellationToken cancellationToken)
        {
