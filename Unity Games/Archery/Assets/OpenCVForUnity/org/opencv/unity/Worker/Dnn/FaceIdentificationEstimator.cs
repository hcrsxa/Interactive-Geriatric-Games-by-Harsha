...
namespace OpenCVForUnity.UnityIntegration.Worker.DnnModule
{
    ...
    /// ```
    /// </summary>
    public class FaceIdentificationEstimator : DnnInferenceWorkerBase
    {
        ...
        public FaceIdentificationEstimator(string fdModelFilepath, string sfModelFilepath, Size inputSize,
                                           ...
                                           int trackBuffer = 30, int backend = Dnn.DNN_BACKEND_OPENCV, int target = Dnn.DNN_TARGET_CPU)
            : base(backend, target)
        {
            ...
            try
            {
                _faceDetector = FaceDetectorYN.create(fdModelFilepath, "", inputSize, _scoreThreshold, _nmsThreshold, _topK, DnnBackend, DnnTarget);
                _faceRecognizer = FaceRecognizerSF.create(sfModelFilepath, "", DnnBackend, DnnTarget);
        ...
        public virtual async Task<Mat> EstimateTaskAsync(Mat image, CancellationToken cancellationToken = default)
        {
        ...
        /// </summary>
        /// <remarks>
        /// <c>@deprecated</c> Use <see cref="EstimateTaskAsync(Mat, CancellationToken)"/>. In a future version, this member will return Unity <c>Awaitable</c> instead of <see cref="Task{TResult}"/>.
        /// See <see cref="EstimateTaskAsync(Mat, CancellationToken)"/>. Web synchronous fallback applies when thread pools are unavailable (OpenCV Dnn inference only).
        /// </remarks>
        [Obsolete("Use EstimateTaskAsync(). EstimateAsync() will return Awaitable in a future version.")]
        public virtual Task<Mat> EstimateAsync(Mat image, CancellationToken cancellationToken = default) =>
            EstimateTaskAsync(image, cancellationToken);
