...
{
    /// <summary>
    /// DAMO-YOLO object detector implementation.
    /// This class provides functionality for object detection using the DAMO-YOLO model implemented with OpenCV's DNN module.
    /// Referring to:
    /// https://github.com/tinyvision/DAMO-YOLO
    /// https://github.com/tinyvision/DAMO-YOLO/blob/master/tools/demo.py
    ///
    /// [Tested Models]
    /// damoyolo_tinynasL18_Ns.onnx https://idstcv.oss-cn-zhangjiakou.aliyuncs.com/DAMO-YOLO/release_model/onnx/before_distill/damoyolo_tinynasL18_Ns.onnx
    /// damoyolo_tinynasL18_Nm.onnx https://idstcv.oss-cn-zhangjiakou.aliyuncs.com/DAMO-YOLO/release_model/onnx/before_distill/damoyolo_tinynasL18_Nm.onnx
    /// damoyolo_tinynasL20_T_436.onnx https://idstcv.oss-cn-zhangjiakou.aliyuncs.com/DAMO-YOLO/release_model/onnx/damoyolo_tinynasL20_T_436.onnx
    /// </summary>
    public class DAMOYOLOObjectDetector : DnnInferenceWorkerBase
    {
        ...
        protected float[] _allBoxesBuffer;
        protected float[] _allClassConfidencesBuffer;
        ...
        /// <summary>
        /// Initializes a new instance of the DAMOYOLOObjectDetector class.
        /// </summary>
...
#endif
        /// </param>
        public DAMOYOLOObjectDetector(string modelFilepath, string classesFilepath, Size inputSize, float confThreshold = 0.35f, float nmsThreshold = 0.6f, int topK = 300, int backend = Dnn.DNN_BACKEND_OPENCV, int target = Dnn.DNN_TARGET_CPU)
            : base(backend, target)
        {
        ...
        protected override Mat[] RunCoreProcessing(Mat[] inputs)
        {
            ...
            // Postprocess
            Mat submat = PostProcess(_forwardOutputList, image.sizeAsValueTuple()); // submat of _output0Buffer is returned
        ...
        protected virtual Mat PreProcess(Mat image)
        {
            ...
            // DAMO-YOLO is scalefactor = 1.0, swapRB = true
            Mat blob = Dnn.blobFromImage(_paddedImg, 1.0, _inputSize, SCALAR_0, true, false, CvType.CV_32F); // HWC to NCHW, BGR to RGB
        ...
        {
            Mat outputBlob_0 = outputBlobs[0]; // 1 * N * 4
            Mat outputBlob_1 = outputBlobs[1]; // 1 * N * _numClasses

            if (outputBlob_1.size(2) != _numClasses)
            {
                Debug.LogWarning("The number of classes and output shapes are different. " +
                                "( outputBlob_1.size(2):" + outputBlob_1.size(2) + " != numClasses:" + _numClasses + " )\n" +
                                "When using a custom model, be sure to set the correct number of classes by loading the appropriate custom classesFile.");

                _numClasses = outputBlob_1.size(2);
            }

            // pre-NMS
            int num = outputBlob_0.size(1);
            using (Mat outputBlob_0_NxBox = outputBlob_0.reshape(1, num))
            using (Mat outputBlob_1_NxClassConfidences = outputBlob_1.reshape(1, num))
            {
                PreNMS(outputBlob_0_NxBox, outputBlob_1_NxClassConfidences, ref _preNMS_Nx6, _confThreshold, _topK);
            }
        ...
        protected virtual void PreNMS(Mat box, Mat classConfidences, ref Mat preNMS_Nx6, float score_threshold, int top_k = 300)
        {
            if (!box.isContinuous())
                throw new ArgumentException("box is not continuous.");
            if (!classConfidences.isContinuous())
                throw new ArgumentException("classConfidences is not continuous.");

            int box_rows = box.rows();
...
#if NET_STANDARD_2_1 && !OPENCV_DONT_USE_UNSAFE_CODE
            ReadOnlySpan<float> allBoxes = box.AsSpan<float>();
            ReadOnlySpan<float> allClassConfidences = classConfidences.AsSpan<float>();
            Span<float> allPreNMS = preNMS_Nx6.AsSpan<float>();
#else
            int requiredBoxesLen = box_rows * BOUNDING_BOX_COLUMNS;
            int requiredClassConfidencesLen = box_rows * _numClasses;
            int requiredPreNMSLen = num_preNMS * DETECTION_RESULT_COLUMNS;

            if (_allBoxesBuffer == null || _allBoxesBuffer.Length < requiredBoxesLen)
                _allBoxesBuffer = new float[requiredBoxesLen];
            if (_allClassConfidencesBuffer == null || _allClassConfidencesBuffer.Length < requiredClassConfidencesLen)
                _allClassConfidencesBuffer = new float[requiredClassConfidencesLen];
            if (_allPreNMSBuffer == null || _allPreNMSBuffer.Length < requiredPreNMSLen)
                _allPreNMSBuffer = new float[requiredPreNMSLen];

            box.get(0, 0, _allBoxesBuffer);
            classConfidences.get(0, 0, _allClassConfidencesBuffer);
            preNMS_Nx6.get(0, 0, _allPreNMSBuffer);
            float[] allBoxes = _allBoxesBuffer;
            float[] allClassConfidences = _allClassConfidencesBuffer;
            ...
            for (int i = 0; i < box_rows; ++i)
            {
                ...
                int maxIdx = -1;
                int baseIdx = i * _numClasses;
                for (int j = 0; j < _numClasses; ++j)
                {
                    float conf = allClassConfidences[baseIdx + j];
                    if (conf > maxVal)
                    {
                        maxVal = conf;
                        maxIdx = j;
                ...
                if (maxVal > score_threshold)
                {
                    ...
                    int boxIdx = i * BOUNDING_BOX_COLUMNS;
                    int preNMSIdx = ind * DETECTION_RESULT_COLUMNS;

#if NET_STANDARD_2_1 && !OPENCV_DONT_USE_UNSAFE_CODE
                    allBoxes.Slice(boxIdx, BOUNDING_BOX_COLUMNS).CopyTo(allPreNMS.Slice(preNMSIdx, BOUNDING_BOX_COLUMNS));
#else
                    Buffer.BlockCopy(allBoxes, boxIdx * 4, allPreNMS, preNMSIdx * 4, BOUNDING_BOX_COLUMNS * 4);
#endif
...
#if !NET_STANDARD_2_1 || OPENCV_DONT_USE_UNSAFE_CODE
                _allBoxesBuffer = null;
                _allClassConfidencesBuffer = null;
