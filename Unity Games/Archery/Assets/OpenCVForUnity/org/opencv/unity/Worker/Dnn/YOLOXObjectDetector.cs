...
{
    /// <summary>
    /// YOLOX object detector implementation.
    /// This class provides functionality for object detection using the YOLOX model implemented with OpenCV's DNN module.
    /// Referring to:
    ...
    /// </summary>
    public class YOLOXObjectDetector : DnnInferenceWorkerBase
    {
        ...
        protected static readonly int[] STRIDES = new int[] { 8, 16, 32 };
        ...
        protected MultiBackendNet _objectDetectionNet;
        ...
        protected Mat _grids;
        protected Mat _expandedStrides;
        ...
        protected float[] _allBoxConfClsconfsBuffer;
        ...
        /// <summary>
        /// Initializes a new instance of the YOLOXObjectDetector class.
        /// </summary>
        /// <param name="modelFilepath">Path to the model file. For OpenCV, typically an ONNX file; when <see cref="MultiBackendDnn.DNN_BACKEND_UNITY_SENTIS"/>, a serialized path that <see cref="Unity.InferenceEngine.ModelLoader.Load(string)"/> can load.</param>
        ...
        /// When <see cref="MultiBackendDnn.DNN_BACKEND_UNITY_SENTIS"/>, <paramref name="target"/> is interpreted as an integer <see cref="Unity.InferenceEngine.BackendType"/> value. Assumes Unity Inference Engine (com.unity.ai.inference) 2.6.1 or newer.
#else
        ...
        /// <param name="target">
#if OPENCV_SENTIS_AVAILABLE
        /// Preferred DNN target (OpenCV <c>DNN_TARGET_*</c>), or if <paramref name="backend"/> is <see cref="MultiBackendDnn.DNN_BACKEND_UNITY_SENTIS"/>, an integer to cast to <see cref="Unity.InferenceEngine.BackendType"/>.
#else
        /// An OpenCV <see cref="Dnn"/> <c>DNN_TARGET_*</c> constant.
#endif
        /// </param>
        public YOLOXObjectDetector(string modelFilepath, string classesFilepath, Size inputSize,
            ...
            : base(backend, target)
        {
            ...
            try
            {
                _objectDetectionNet = MultiBackendDnn.readNet(modelFilepath);
                _objectDetectionNet.setPreferableBackend(DnnBackend);
                _objectDetectionNet.setPreferableTarget(DnnTarget);
                cachedUnconnectedOutLayersNames = _objectDetectionNet.getUnconnectedOutLayersNames();
            }
            catch (Exception e)
            {
            ...
            GenerateAnchors(out _grids, out _expandedStrides);
        }
        ...
        ///
        /// For better performance in async scenarios, use DetectTaskAsync instead.
        /// </remarks>
        ...
        public virtual async Task<Mat> DetectTaskAsync(Mat image, CancellationToken cancellationToken = default)
        {
        ...
        public virtual Task<Mat> DetectAsync(Mat image, CancellationToken cancellationToken = default) =>
        ...
        protected override Mat[] RunCoreProcessing(Mat[] inputs)
        {
            ...
            using (Mat inputBlob = PreProcess(image))
            {
                ...
                _objectDetectionNet.forward(_forwardOutputList, _cachedUnconnectedOutLayersNames);
            }

            Mat submat = PostProcess(_forwardOutputList, image.sizeAsValueTuple());
...
#if OPENCV_SENTIS_AVAILABLE
            if (_objectDetectionNet.UsesSentis)
            {
                ThrowIfDisposed();
                ...
                using (Mat inputBlob = PreProcess(image))
                {
                    _forwardOutputList.Clear();
                    _objectDetectionNet.setInput(inputBlob);
                    await _objectDetectionNet.forwardTaskAsync(_forwardOutputList, _cachedUnconnectedOutLayersNames, cancellationToken);
                }
        ...
        protected virtual Mat PreProcess(Mat image)
        {
            ...
            using (Mat _paddedImg_roi = new Mat(_paddedImg, (0, 0, image.cols(), image.rows())))
            {
                image.copyTo(_paddedImg_roi);
            }

            // Create a 4D blob from a frame.
            // YOLOX is scalefactor = 1.0, swapRB = false
            Mat blob = Dnn.blobFromImage(_paddedImg, 1.0, _inputSize, SCALAR_0, false, false, CvType.CV_32F); // HWC to NCHW, BGR
        ...
        {
            Mat outputBlob_0 = outputBlobs[0]; // 1 * N * (5 + _numClasses)

            if (outputBlob_0.size(2) != 5 + _numClasses)
            {
                Debug.LogWarning("The number of classes and output shapes are different. " +
                                "( outputBlob_0.size(2):" + outputBlob_0.size(2) + " != 5 + numClasses:" + _numClasses + " )\n" +
                                "When using a custom model, be sure to set the correct number of classes by loading the appropriate custom classesFile.");

                _numClasses = outputBlob_0.size(2) - 5;
            }

            int num = outputBlob_0.size(1);
            using (Mat outputBlob_0_NxBoxConfClsconfs = outputBlob_0.reshape(1, num))
            {
                // Decode boxes from YOLOX output format:
                // 1. Add grid offsets to center coordinates (xy)
                // 2. Multiply center coordinates by stride
                // 3. Exponentiate width/height predictions
                // 4. Multiply width/height by stride
                using (Mat box_cols = outputBlob_0_NxBoxConfClsconfs.colRange((0, 4)))
                using (Mat _box_cols_cxy = box_cols.colRange((0, 2)))
                using (Mat _box_cols_wh = box_cols.colRange((2, 4)))
                using (Mat _grids_Nx2 = _grids.reshape(1, num))
                using (Mat _expanded_strides_Nx2 = _expandedStrides.reshape(1, num))
                {
                    Core.add(_box_cols_cxy, _grids_Nx2, _box_cols_cxy);
                    Core.multiply(_box_cols_cxy, _expanded_strides_Nx2, _box_cols_cxy);
                    Core.exp(_box_cols_wh, _box_cols_wh);
                    Core.multiply(_box_cols_wh, _expanded_strides_Nx2, _box_cols_wh);
                }

                // pre-NMS
                PreNMS_BoxConfClsconfs_CXCYWHtoX1Y1X2Y2(outputBlob_0_NxBoxConfClsconfs, ref _preNMS_Nx6, _confThreshold, _topK);
            }
            ...
            using (MatOfInt indices = new MatOfInt())
            {
                ...
                lock (_lockObject)
                {
                    ...
                    // Scale boxes
                    ScaleBoxes_PaddingOnBottomRight(result, _inputSize.ToValueTuple(), originalSize);
                }
            }
        ...
        protected virtual void PreNMS_BoxConfClsconfs_CXCYWHtoX1Y1X2Y2(Mat boxConfClsconfs, ref Mat preNMS_Nx6, float score_threshold, int top_k = 300)
        {
            if (!boxConfClsconfs.isContinuous())
                throw new ArgumentException("boxConfClsconfs is not continuous.");

            const int CONFIDENCE_INDEX = 4;
            const int CLASS_CONFIDENCES_INDEX = 5;

            int boxConfClsconfs_rows = boxConfClsconfs.rows();
            int boxConfClsconfs_cols = boxConfClsconfs.cols();
...
#if NET_STANDARD_2_1 && !OPENCV_DONT_USE_UNSAFE_CODE
            ReadOnlySpan<float> allBoxConfClsconfs = boxConfClsconfs.AsSpan<float>();
            Span<float> allPreNMS = preNMS_Nx6.AsSpan<float>();
#else
            int requiredBoxConfClsconfsLen = boxConfClsconfs_rows * boxConfClsconfs_cols;
            int requiredPreNMSLen = num_preNMS * DETECTION_RESULT_COLUMNS;
            if (_allBoxConfClsconfsBuffer == null || _allBoxConfClsconfsBuffer.Length < requiredBoxConfClsconfsLen)
                _allBoxConfClsconfsBuffer = new float[requiredBoxConfClsconfsLen];
            if (_allPreNMSBuffer == null || _allPreNMSBuffer.Length < requiredPreNMSLen)
                _allPreNMSBuffer = new float[requiredPreNMSLen];

            boxConfClsconfs.get(0, 0, _allBoxConfClsconfsBuffer);
            preNMS_Nx6.get(0, 0, _allPreNMSBuffer);
            float[] allBoxConfClsconfs = _allBoxConfClsconfsBuffer;
            float[] allPreNMS = _allPreNMSBuffer;
#endif

            int ind = 0;
            for (int i = 0; i < boxConfClsconfs_rows; ++i)
            {
                float confidence = allBoxConfClsconfs[i * boxConfClsconfs_cols + CONFIDENCE_INDEX];
                if (confidence > score_threshold)
                {
                    ...
                    int maxIdx = -1;
                    int baseIdx = i * boxConfClsconfs_cols + CLASS_CONFIDENCES_INDEX;
                    for (int j = 0; j < _numClasses; ++j)
                    {
                        float conf = allBoxConfClsconfs[baseIdx + j];
                        if (conf > maxVal)
                        {
                            maxVal = conf;
                            maxIdx = j;
                    ...
                    int boxIdx = i * boxConfClsconfs_cols;
                    int preNMSIdx = ind * DETECTION_RESULT_COLUMNS;

                    // Convert from [cx,cy,w,h] to [x1,y1,x2,y2]
                    float cx = allBoxConfClsconfs[boxIdx];
                    float cy = allBoxConfClsconfs[boxIdx + 1];
                    float w = allBoxConfClsconfs[boxIdx + 2];
                    float h = allBoxConfClsconfs[boxIdx + 3];
                    ...
                    allPreNMS[preNMSIdx + DETECTION_CONFIDENCE_INDEX] = maxVal * confidence;
        ...
        protected virtual void ScaleBoxes_PaddingOnBottomRight(Mat result, (double width, double height) inputSize,
        ...
        protected virtual void GenerateAnchors(out Mat grids, out Mat expandedStrides)
        {
            int num = 0;
            ...
            {
                hsizes[i] = (int)(_inputSize.height / STRIDES[i]);
                wsizes[i] = (int)(_inputSize.width / STRIDES[i]);

                num += hsizes[i] * wsizes[i];
            }

            grids = new Mat(new int[] { 1, num, 2 }, CvType.CV_32FC1);
            expandedStrides = new Mat(new int[] { 1, num, 2 }, CvType.CV_32FC1);

            using (Mat grids_Nx2 = grids.reshape(1, num))//num*2*CV_32FC1
            using (Mat expanded_strides_Nx2 = expandedStrides.reshape(1, num))//num*2*CV_32FC1
            {
                int index = 0;
                ...
                for (int i = 0; i < STRIDES.Length; i++)
                {
                    ...
                    using (Mat w_arange = Arange(0, wsize))
                    using (Mat h_arange = Arange(0, hsize).t())
                    using (Mat xv = new Mat(hsize, wsize, CvType.CV_32FC1))
                    using (Mat yv = new Mat(hsize, wsize, CvType.CV_32FC1))
                    {
                        ...
                        using (Mat grid_roi = new Mat(grids_Nx2, (0, index, 1, (int)xv.total())))//total*1*CV_32FC1
                        {
                            xv_totalx1.copyTo(grid_roi);
                        }
                        using (Mat yv_totalx1 = yv.reshape(1, (int)yv.total()))//total*1*CV_32FC1
                        using (Mat grid_roi = new Mat(grids_Nx2, (1, index, 1, (int)yv.total())))//total*1*CV_32FC1
                        {
                        ...
                        using (Mat expanded_strides_roi = expanded_strides_Nx2.rowRange(index, index + shape))
                        {
                            Imgproc.rectangle(expanded_strides_roi, (0, 0, 2, shape), (stride, stride, stride, stride));
                        }
                    }
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            if (disposing)
            {
                _objectDetectionNet?.Dispose();
                _objectDetectionNet = null;
                _forwardOutputList.Clear();
                _grids?.Dispose(); _grids = null;
                _expandedStrides?.Dispose(); _expandedStrides = null;
...
#if !NET_STANDARD_2_1 || OPENCV_DONT_USE_UNSAFE_CODE
                _allBoxConfClsconfsBuffer = null;
