...
namespace OpenCVForUnity.UnityIntegration.Worker.DnnModule
{
    ...
    /// </summary>
    public class YuNetV2FaceDetector : DnnInferenceWorkerBase
    {
            ...
            : base(backend, target)
        {
            ...
            try
            {
                _detectionModel = FaceDetectorYN.create(modelFilepath, configFilepath, _inputSize, _confThreshold, _nmsThreshold, _topK, DnnBackend, DnnTarget);
            }
            catch (Exception e)
            {
        ...
        /// See <see cref="DetectTaskAsync(Mat, CancellationToken)"/>. Web synchronous fallback applies when thread pools are unavailable (OpenCV Dnn inference only).
        /// </remarks>
        /// <param name="image">Input image in BGR format.</param>
        /// <param name="cancellationToken">Optional token to cancel the operation.</param>
